...
/// <summary>
/// LazyListBox 类，继承 ListBox 控件，实现懒加载和搜索功能。
/// </summary>
public class LazyListBox : ListBox
{
    ...
    public static readonly DependencyProperty LazyDataSourceProperty =
        DependencyProperty.Register(nameof(LazyDataSource), typeof(IEnumerable), typeof(LazyListBox),
    ...
    public static readonly DependencyProperty SearchTextProperty =
        DependencyProperty.Register(nameof(SearchText), typeof(string), typeof(LazyListBox),
            new PropertyMetadata(string.Empty, OnSearchTextChanged));

    /// <summary>
    /// 定义 FirstLoadCount 依赖属性，默认值 20。
    /// </summary>
    public static readonly DependencyProperty FirstLoadCountProperty =
        DependencyProperty.Register(nameof(FirstLoadCount), typeof(int), typeof(LazyListBox),
    ...
    public LazyListBox()
    {
        ...
        this.Loaded += (s, e) => this.LoadNextItem(this.FirstLoadCount); // 初始加载20项
    }
    ...
    {
        if (d is LazyListBox lazyListBox && e.NewValue is IEnumerable newValue)
        {
            lazyListBox.ResetLazyLoading();
        }
    }
    ...
    {
        if (d is LazyListBox lazyListBox && e.NewValue is string newValue)
        {
            lazyListBox.ResetLazyLoading();
        }
    }
